...
export const wordpressAdminEn: BlogPost = {
  ...
  lead: [
    "The WordPress admin is the panel where you publish posts, edit pages, and install plugins. Usual entry: `https://your-site.ru/wp-admin/` or `/wp-login.php`.",
    "Below — how to open the login form, what to do with a forgotten password, when to touch the database on hosting, and why cookies/cache get in the way. This is not a guide to hacking other people’s sites and not a call to live with the login `admin`.",
  ],
  faq: [
    {
    ...
    },
    {
      ...
      a: "Different login URL, HTTPS redirect, protection plugin, cache, wrong domain (www/non-www), or the site is down. Check the address from the host/developer email.",
    },
    {
    ...
    },
    {
      ...
      a: "Old guides said MD5. Modern WordPress uses stronger hashing; host panels often have a “reset WP password” without hand MD5. Check current docs.",
    },
  ],
  sections: [
    {
      ...
      level: 2,
      paras: [
        "In the address bar open `https://your-domain.ru/wp-admin/`. If the form doesn’t show, try `https://your-domain.ru/wp-login.php`. Use your domain; prefer HTTPS.",
        "After WordPress install, login and password are set at the install step (or come from the developer/host). “Remember me” is fine on a personal device and risky on a shared one.",
      ],
      lists: [
        {
    ...
    },
    {
      ...
      paras: [
        ...
        "Mail sometimes lands in spam or never sends because of host mail settings. Then check the mailbox from `wp-config`/the user profile or ask hosting/the developer to reset with their tool.",
      ],
      lists: [
        {
          intro: "Quick order:",
    ...
    },
    {
      ...
      level: 2,
      paras: [
        "Many hosts can reset the WordPress password from their panel — prefer that over hand edits. If you go into phpMyAdmin: back up the DB, find the users table (often `wp_users`; the prefix may differ), update the password the way current WordPress/host docs recommend.",
    ...
    },
    {
      ...
      level: 2,
      paras: [
        "Change a weak password, check the user list and extra accounts, update WordPress/themes/plugins. For ongoing work a password manager and 2FA help.",
